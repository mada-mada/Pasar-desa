...
<div class="relative bg-blue-deep pt-16 pb-24 border-b-4 border-gold z-10">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 class="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight mb-4">
            Eksplorasi <span class="text-gold text-transparent bg-clip-text bg-gradient-to-r from-gold to-yellow-200">Pasar Desa</span> Indramayu
        </h1>
        <p class="mt-4 max-w-2xl text-xl text-blue-100 mx-auto mb-10">
            Temukan lokasi, jadwal pasaran, dan fasilitas dari seluruh pasar desa di Kabupaten Indramayu dengan mudah dan cepat.
        </p>
        ...
        <form action="{{ route('pasar.index') }}" method="GET" class="max-w-3xl mx-auto">
            <div class="relative flex items-center">
                ...
                <button type="submit" class="absolute right-2 px-6 py-2 rounded-full bg-gold hover:bg-yellow-500 text-blue-deep font-bold transition-all shadow-lg transform hover:scale-105">
                    Cari Pasar
                </button>
            </div>
        </form>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-end mb-8 border-b-2 border-gray-200 pb-4">
        <div>
            <h2 class="text-3xl font-bold text-gray-900"><span class="border-l-4 border-gold pl-3">Daftar</span> Pasar Desa</h2>
            @if(request('search'))
                <p class="text-gray-500 mt-2">Hasil pencarian untuk: <span class="font-bold text-blue-deep">"{{ request('search') }}"</span></p>
            @endif
        </div>
    </div>
            ...
            @foreach($Pasar as $p)
                <div class="bg-white rounded-xl shadow-md overflow-hidden card-hover border border-gray-100 flex flex-col h-full transform transition-all duration-300">
                    ...
                    <div class="p-6 flex-grow flex flex-col">
                        <h3 class="font-bold text-xl text-gray-900 mb-2 truncate card-title transition-colors">{{ $p->nama_pasar }}</h3>
                    ...
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 mt-auto">
                        <a href="{{ route('pasar.show', $p->id) }}" class="block w-full text-center py-2 rounded-lg bg-blue-deep hover:bg-blue-dark text-white font-semibold transition-colors shadow hover:shadow-lg">
...
<div class="bg-gray-100 py-16 border-t border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-end mb-10">
            <div>
                <h2 class="text-3xl font-bold text-gray-900"><span class="border-l-4 border-gold pl-3">Artikel</span> Terkini</h2>
                <p class="text-gray-600 mt-2">Informasi dan berita seputar perkembangan perekonomian desa.</p>
            </div>
            <a href="{{ route('artikel.index') }}" class="hidden md:flex items-center text-blue-deep font-bold hover:text-gold transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <article class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden transform hover:-translate-y-2 transition-all duration-300">
                @if($art->foto_thumbnail)
                    <img src="{{ asset('storage/' . $art->foto_thumbnail) }}" class="w-full h-48 object-cover" alt="{{ $art->judul }}">
                @else
                ...
                @endif
                <div class="p-6">
                    <div class="text-xs text-gold font-bold mb-2 uppercase">{{ \Carbon\Carbon::parse($art->tanggal_rilis)->translatedFormat('d F Y') }}</div>
                    <a href="{{ route('artikel.show', $art->id) }}" class="block mt-2">
                        <h3 class="text-xl font-bold text-gray-900 hover:text-blue-deep transition-colors line-clamp-2">{{ $art->judul }}</h3>
                    </a>
        ...
        <div class="mt-8 text-center md:hidden">
            <a href="{{ route('artikel.index') }}" class="inline-block px-6 py-3 rounded-lg border-2 border-blue-deep text-blue-deep font-bold hover:bg-blue-deep hover:text-white transition-colors">
                Lihat Semua Berita
            </a>
        </div>
    </div>
