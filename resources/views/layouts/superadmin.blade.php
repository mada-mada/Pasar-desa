...
<body class="bg-gray-100 flex h-screen overflow-hidden">
    ...
    <!-- Sidebar -->
    <div id="sidebar-overlay" class="fixed inset-0 bg-black opacity-50 z-20 hidden xl:hidden"></div>
    <aside id="sidebar"
        class="w-64 bg-gray-900 text-white flex flex-col shadow-2xl z-30 absolute inset-y-0 left-0 transform -translate-x-full xl:relative xl:translate-x-0 transition duration-300 ease-in-out">
        <div class="h-16 flex items-center justify-center border-b border-gray-800">
            <h1 class="text-xl font-bold text-gold">SuperAdmin</h1>
        </div>
        <nav class="flex-1 overflow-y-auto py-4">
            <ul class="space-y-1">
                <li>
                    <a href="{{ route('superadmin.pasar.index') }}"
                        class="flex items-center px-6 py-3 hover:bg-gray-800 transition-colors border-l-4 {{ request()->routeIs('superadmin.pasar.*') ? 'border-gold bg-gray-800' : 'border-transparent' }}">
                ...
                <li>
                    <a href="{{ route('superadmin.artikel.index') }}"
                        ...
                        <span>Awasi Artikel</span>
                    </a>
                </li>
                <li>
                    <a href="{{ route('superadmin.admin.index') }}"
                        class="flex items-center px-6 py-3 hover:bg-gray-800 transition-colors border-l-4 {{ request()->routeIs('superadmin.admin.*') ? 'border-gold bg-gray-800' : 'border-transparent' }}">
                        <i class="fas fa-users-cog w-6 text-gold"></i>
        ...
        <div class="p-4 border-t border-gray-800">
            <form method="POST" action="/logout">
                @csrf
                <button type="submit"
                    class="w-full flex items-center justify-center px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg text-white transition-colors">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-6 z-10 glass">
            <div class="flex items-center">
                <button id="mobile-menu-btn" class="text-gray-500 hover:text-gray-900 focus:outline-none xl:hidden">
